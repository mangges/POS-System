<div class="payment-modal-card">
    ...
    <div class="modal-left-content">
        ...
        <div class="modal-header">
            <h3 class="modal-title">{{ __('pos.Metode Pembayaran') }}</h3>
        ...
        <div class="payment-methods-grid">
            ...
            <label class="method-option {{ $this->paymentMethod === 'cash' ? 'active' : '' }}">
                ...
                <span class="method-label label-cash">{{ __('pos.Tunai') }}</span>
            </label>
            @endif
            ...
            <label class="method-option {{ $this->paymentMethod === 'transfer' ? 'active' : '' }}">
                ...
                <span class="method-label label-gray">{{ __('pos.Debit/Transfer') }}</span>
            </label>
            @endif
        </div>

        <div class="order-type-section">
            <h4 class="section-subtitle">{{ __('pos.Tipe Pesanan') }}</h4>
            <div class="order-type-grid">
                <label class="method-option {{ $this->orderType === 'dine-in' ? 'active' : '' }}">
                    ...
                    <span class="method-label label-cash">{{ __('pos.Makan ditempat') }}</span>
                </label>
                <label class="method-option {{ $this->orderType === 'takeaway' ? 'active' : '' }}">
                    ...
                    <span class="method-label label-gray">{{ __('pos.Bungkus') }}</span>
                </label>
            </div>
        </div>
        ...
        @if($this->paymentMethod !== 'cash')
            @if($this->qrisImageForAmount($displayTotal))
                <div class="payment-notice">
                    ...
                    <p>{{ __('pos.Minta pelanggan scan QR ini untuk membayar') }} Rp {{ number_format($displayTotal) }}.</p>
                </div>
                ...
                <button type="button" wire:click="openQrisPreviewModal" class="btn-preview-qris">
                    <i class="bi bi-arrows-fullscreen"></i> {{ __('pos.Tampilkan QRIS ke Pelanggan') }}
                </button>
            @else
                <div class="payment-notice">
                    <i class="bi bi-exclamation-circle"></i>
                    <p>{{ __('pos.Untuk metode pembayaran selain tunai, silakan selesaikan pembayaran melalui aplikasi terkait') }}</p>
                </div>
            @endif

            <label class="payment-confirm-checkbox">
                <input type="checkbox" wire:model.live="paymentConfirmed">
                <span>{{ __('pos.Konfirmasi pembayaran diterima') }}</span>
            </label>
        @endif

        @if($this->paymentMethod === 'cash')
        <div id="cashDenominations">
            <h4 class="section-subtitle">{{ __('pos.Uang Pecahan') }}</h4>
        ...
        <div class="input-received-group">
            <label class="input-label">{{ __('pos.Uang Diterima') }}</label>
    ...
    <div class="modal-right-content">
        <div class="summary-top-wrapper">
            ...
            <h3 class="summary-title"></h3>{{ __('pos.Ringkasan Transaksi') }}

            <div class="price-details">
                <div class="price-row text-secondary">
                    <span>{{ __('pos.Pelanggan') }}</span>
            ...
            <div class="@if($isInsufficient) change-box-danger @else change-box-success @endif">
                <div class="@if($isInsufficient) change-title-danger @else change-title-success @endif">{{ __('pos.Kembalian') }}</div>

                @if($isInsufficient)
                    <p class="change-notice-danger">{{ __('pos.Jumlah uang diterima kurang') }}</p>
                @endif
        ...
        <div class="submit-action-wrapper">
            <button type="button" wire:click="finalizeOrder" class="btn-submit-payment" @if(! $canFinalize) disabled @endif>
                ...
                <span>{{ __('pos.Selesai & Cetak Struk') }}</span>
            </button>
        </div>
    </div>
</div>